...
from backend.service.RegistryService import get_rule_engine_service
from backend.service.InterventionService import get_intervention_service
...
def get_rules():
    """获取规则列表"""
    try:
        logger.info("Received request to get rules")
        page = request.args.get('page', 1, type=int)
        page_size = request.args.get('page_size', 20, type=int)
        is_active = request.args.get('is_active')
        rule_type = request.args.get('rule_type')
        
        logger.info(f"Request params: page={page}, page_size={page_size}, is_active={is_active}, rule_type={rule_type}")
        ...
        logger.info(f"Returning {len(rules)} rules, total={total}")
...
def create_rule():
    ...
    try:
        ...
        engine = get_rule_engine_service()
        rule = engine.create_rule_detailed(
...
def update_rule(rule_id):
    ...
    try:
        ...
        engine = get_rule_engine_service()
        rule = engine.update_rule_detailed(
            rule_id=rule_id,
...
def get_notification_preferences():
    ...
    try:
        ...
        # 通过service层获取通知偏好
        engine = get_rule_engine_service()
        prefs = engine.get_user_notification_preferences(user_id)
...
def update_notification_preferences():
    ...
    try:
        ...
        # 通过service层更新通知偏好
        engine = get_rule_engine_service()
        engine.update_user_notification_preferences(user_id, data)
