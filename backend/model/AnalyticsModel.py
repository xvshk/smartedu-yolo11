...
from backend.model.InterfaceModel import IAnalyticsRepository
...
class AnalyticsRepository(IAnalyticsRepository):
    ...
    def get_behavior_statistics(
        ...
        if session_id:
            conditions.append("dr.session_id = %s")
        ...
        sql = f"""
            ...
            {where_clause}
            GROUP BY be.class_name, be.behavior_type
        """
        results = self.db.query(sql, tuple(params)) if params else self.db.query(sql)
        ...
        for row in results:
            behavior_counts[row['class_name']] = row['count']
            if row['behavior_type'] == 'normal':
                normal_count += row['count']
            else:
                warning_count += row['count']
        
        total = normal_count + warning_count
        ...
        return {
            ...
            'total_count': total,
            'attention_rate': normal_count / total if total > 0 else 1.0
        }
    ...
    def get_alert_statistics(
        ...
        for row in results:
            ...
            count = row['count']
            alert_distribution[level] = count
            if level > 0:
                total_alerts += count
        ...
        return {
            ...
            'level_0_count': alert_distribution[0],
            'level_1_count': alert_distribution[1],
            'level_2_count': alert_distribution[2],
            'level_3_count': alert_distribution[3]
        }
